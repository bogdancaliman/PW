...
const categorii = ['Introducerea in management', 'Luarea deciziei şi leadership-ul'];
const cursuriCategorie = {
	...
	'Introducerea in management': [
		'Definirea organizatiilor',
		'Istoria organizatiilor',
		'Teorii manageriale',
		'Managementul',
	],
	'Luarea deciziei şi leadership-ul': [
		'Definirea si clasificarea deciziilor',
		...
		<li className={styles.link}>
			<Link href={`/cursuri/${numelink}`}>
				<a
					...
					className={`${lastcurs !== undefined && lastcurs === nume ? styles.curs_selectat : ''}`}
				>
					{nume}
				</a>
			</Link>
...
function Categorie({ nume, deschis = false }) {
	...
	function showCursuri() {
		return cursuriCategorie[nume].map((numeCurs) => <LinkCatreCurs nume={numeCurs} />);
	}
	return (
		<>
			<h2 className={styles.categorie} onClick={() => setAratCursuri(!aratCursuri)}>
				{nume}
			</h2>
